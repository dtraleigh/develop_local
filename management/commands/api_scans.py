...
def development_api_scan():
    ...
    # All developments after 2010.
    url = "https://services.arcgis.com/v400IkDOw1ad7Yad/arcgis/rest/services/Development_Plans/FeatureServer/0/query?where=submitted_yr>2010&outFields=*&outSR=4326&f=json"
    devplan_data = get_api_json(url)

    for dev_plan in devplan_data["features"]:
        attribute_data = dev_plan["attributes"]
        geometry_data = dev_plan["geometry"]
        ...
        if DevelopmentPlan.objects.filter(objectid=attribute_data["OBJECTID"]).exists():
            ...
            if api_object_is_different(known_dev_object, attribute_data):
                ...
                known_dev_object.daystoappr = clean_unix_date(["daystoapprove"])
                known_dev_object.plan_type = clean_unix_date(attribute_data["plan_type"])
                known_dev_object.status = clean_unix_date(attribute_data["status"])
                known_dev_object.appealperi = attribute_data["appealperiodends"]
                known_dev_object.updated = attribute_data["updated"]
                known_dev_object.sunset_dat = attribute_data["sunset_date"]
        ...
        # If we don't know about it, we need to add it
            ...
            DevelopmentPlan.objects.create(objectid=attribute_data["OBJECTID"],
                                           ...
                                           geom=Point(geometry_data["x"], geometry_data["y"]))
